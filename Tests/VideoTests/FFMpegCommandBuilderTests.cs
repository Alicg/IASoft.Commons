...
namespace VideoTests
{
    ...
    public class FFMpegCommandBuilderTests
    {
        ...
        public void DrawTextTest()
        {
            ...
            Assert.AreEqual("[in]drawtext=enable='between(t,1,3)':fontfile=PathToFonts:text='Text1':fontsize=10:fontcolor=red:x=(main_w/2-text_w/2):y=main_h-(text_h*(1 -0)) - 15[text_out]", textFilter);
        }
        ...
        public void DrawImageTest()
        {
            ...
            Assert.AreEqual("[in][0:v]overlay=0:0:enable='between(t,0,4)' [tmp0]", imageFilter);
        }
        ...
        public void ConcatAndDrawTextAndDrawImagesTest()
        {
            ...
            Assert.AreEqual("[0:v]setdar=16/9[v0];[1:v]setdar=16/9[v1];[2:v]setdar=16/9[v2];[v0][0:a][v1][1:a][v2][2:a]concat=n=3:v=1:a=1[vv][a];[vv][3:v]overlay=0:0:enable='between(t,0,4)' [tmp3];[tmp3]drawtext=enable='between(t,1,3)':fontfile=PathToFonts:text='Text1':fontsize=10:fontcolor=red:x=(main_w/2-text_w/2):y=main_h-(text_h*(1 -0)) - 15[text_out]", filter);
        }
    }
}